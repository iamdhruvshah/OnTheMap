...
class UdacityAPI {
    ...
    class func postNewStudenLocation(newLatitude: Double, newLongitude: Double, locationString: String, locationMediaURL: String, completion: @escaping ([LocationResults], Error?) -> Void) {
        let postRequestBody = StudentLocation(firstName: UserSession.userId, lastName: UserSession.firstName, uniqueKey: UserSession.lastName, latitude: newLatitude, longitude: newLongitude, medialURL: locationString, mapString: locationMediaURL)
